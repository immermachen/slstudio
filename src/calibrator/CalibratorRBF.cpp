...
CalibrationData CalibratorRBF::calibrate(){
    ...
    //stereo calibration
    cv::Mat Rp, Tp, E, F;
    //opencv2.x to opencv3.x
    //double stereo_error = cv::stereoCalibrate(Q, qc, qp, Kc, kc, Kp, kp, frameSize, Rp, Tp, E, F, cv::TermCriteria(cv::TermCriteria::COUNT + cv::TermCriteria::EPS, 100, DBL_EPSILON), cv::CALIB_FIX_INTRINSIC);
